    ...
    <Series>
      {scenes.map((scene, i) => (
        <Series.Sequence key={i} durationInFrames={scene.durationInFrames}>
          ...
          {scene.type === 'quote'     && <QuoteScene     {...scene} colorScheme={colorScheme} />}
          {scene.type === 'highlight' && <HighlightScene {...scene} colorScheme={colorScheme} />}
        </Series.Sequence>
      ))}
    </Series>
  );
...
export const defaultProps: PromptVideoProps = {
  scenes: [
    ...
    { type: 'highlight', emoji: '🚀', heading: 'Ship Faster', subtext: 'Built for modern teams', durationInFrames: 90 },
